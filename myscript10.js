...
function switch_sensor(){
	
	switch_presence = light.raw_value();
	console.log("Switch Sensor Value"+" "+ switch_presence);
	if(switch_presence<switch_threshold && state==1) 
	{
		console.log('gate open**************');
...
function closingIr(){
	...
	console.log("Vehicle sensor Value"+" "+ vehicle_presence);
	 ...
	 if(vehicle_presence>vehicle_threshold && switch_presence>switch_threshold)
		{   
			pin5.write(0);
			pin8.write(1);
...
setInterval(sensorReading1,2500); 

setInterval(closingIr,500); 
setInterval(switch_sensor,20);
